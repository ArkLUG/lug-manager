...
// Format: "{title} | {start_date}-{end_date} | {location}"
        ...
        if (thread_id.empty() && !lug_msg_id.empty()) {
            ...
            if (!thread_id.empty() && !discord_.get_guild_id().empty()) {
                ...
                std::string updated_content =
                    DiscordClient::build_event_announcement_content(created, role, thread_url);
...
LugEvent EventService::update(int64_t id, const LugEvent& updates) {
    ...
    // Build thread URL for inclusion in edited messages
    std::string thread_url;
    ...
    // Edit the lug_channel announcement message if we have its ID
    if (!updated.discord_lug_message_id.empty() && !discord_.get_lug_channel_id().empty()) {
        try {
            ...
            std::string new_content =
                DiscordClient::build_event_announcement_content(updated, role, thread_url);
            discord_.update_channel_message(
                discord_.get_lug_channel_id(),
                updated.discord_lug_message_id,
                new_content);
        } catch (const std::exception& ex) {
            std::cerr << "[EventService] Warning: failed to update lug announcement for event "
                      << updated.id << ": " << ex.what() << "\n";
        }
    }

    // Edit the chapter announcement message if we have its ID
        ...
        try {
            ...
            if (ch && !ch->discord_announcement_channel_id.empty()) {
                std::string ch_role = ch->discord_member_role_id.empty()
                    ...
                    : ch->discord_member_role_id;
                std::string new_content =
                    DiscordClient::build_event_announcement_content(updated, ch_role, thread_url);
                discord_.update_channel_message(
                    ch->discord_announcement_channel_id,
                    updated.discord_chapter_message_id,
                    new_content);
            }
        } catch (const std::exception& ex) {
            std::cerr << "[EventService] Warning: failed to update chapter announcement for event "
                      << updated.id << ": " << ex.what() << "\n";
        }
    }
