...
static const char* kSelectAllCols =
    ...
    "event_attendance_required, requires_paid_dues, sort_order, created_at, updated_at "
    "FROM perk_levels";
...
PerkLevel PerkLevelRepository::row_to_perk(Statement& stmt) {
    ...
    p.sort_order                  = static_cast<int>(stmt.col_int(6));
    p.created_at                  = stmt.col_text(7);
    p.updated_at                  = stmt.col_text(8);
    return p;
}
...
PerkLevel PerkLevelRepository::create(const PerkLevel& p) {
    auto stmt = db_.prepare(
        ...
        "event_attendance_required, requires_paid_dues, sort_order) VALUES (?,?,?,?,?,?)");
    stmt.bind(1, p.name);
    ...
    stmt.bind(6, static_cast<int64_t>(p.sort_order));
    stmt.step();
...
bool PerkLevelRepository::update(const PerkLevel& p) {
    auto stmt = db_.prepare(
        ...
        "event_attendance_required=?, requires_paid_dues=?, sort_order=?, "
