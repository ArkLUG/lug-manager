...
void register_all_routes(LugApp& app, Services& svc) {
    ...
    register_meeting_routes(app, svc.meetings, svc.attendance, svc.chapter_members);
    register_event_routes(app, svc.events, svc.attendance, svc.chapter_members, svc.discord, svc.members);
    register_attendance_routes(app, svc.attendance, svc.events, svc.meetings);
    register_calendar_routes(app, svc.calendar);
    register_settings_routes(app, svc.settings, svc.discord, svc.member_sync, svc.calendar);
